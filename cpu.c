...
//    dereference_src = 0;
    ...
    switch(opcode.address_mode)
    {
        case ADDRESS_MODE_ABSOLUTE:
            ...
            src_value |= ((uint32_t)reg_dbr) << 16;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_ABSOLUTE_INDEXED_X:
            ...
            src_value = *(uint16_t *)(opcode_address + 1) + reg_x.reg_x;
            src_value |= ((uint32_t)reg_dbr) << 16;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_ABSOLUTE_INDEXED_Y:
            ...
            src_value = *(uint16_t *)(opcode_address + 1) + reg_y.reg_y;
            src_value |= ((uint32_t)reg_dbr) << 16;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_ABSOLUTE_INDIRECT:
            ...
            src_value = (*(uint32_t *)(opcode_address + 1)) & 0x00ffffff;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_ACCUMULATOR:
            ...
            operands[0] = &reg_accum.reg_accumC;
        ...
        case ADDRESS_MODE_BLOCK_MOVE:
            ...
            dst_value = (((uint32_t)reg_dbr) << 16) & 0x00ffffff;
            dst_value |= reg_y.reg_y;

            src_value = (((uint32_t)opcode_address[2]) << 16) & 0x00ffffff;
            src_value |= reg_x.reg_x;
        break;
        ...
        case ADDRESS_MODE_DIRECT_INDEXED_INDIRECT:
            ...
            reg_x.reg_x = *(uint16_t *)memory_pointer(src_value);
            src_value = ((uint32_t)reg_x.reg_x) | (((uint32_t)reg_dbr) << 16);
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_DIRECT_INDIRECT_INDEXED:
            ...
            src_value = *(uint16_t *)memory_pointer((unsigned int )reg_d);
            src_value |= (((uint32_t)reg_dbr) << 16) & 0x00ffffff;
        ...
        case ADDRESS_MODE_DIRECT_INDIRECT_LONG_INDEXED:
            ...
            src_value = *(uint32_t *)memory_pointer(src_value) & 0x00ffffff;
            src_value += reg_y.reg_y;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_DIRECT_INDIRECT:
            ...
            src_value = opcode_address[1] + reg_d;
            src_value = *(uint16_t *)memory_pointer(src_value);
            src_value |= (((uint32_t)reg_dbr) << 16) & 0x00ffffff;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_DIRECT:
            ...
            src_value = (uint32_t)(opcode_address[1] + reg_d);
            src_value &= 0x0000ffff;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
        break;
        ...
        case ADDRESS_MODE_IMMEDIATE:
            ...
            src_value = *(uint16_t *)(opcode_address + 1);
        ...
        case ADDRESS_MODE_STACK_RELATIVE_INDIRECT_INDEXED:
            ...
            src_value = reg_s + opcode_address[1];
            src_value = *(uint32_t *)memory_pointer(src_value);
            src_value |= (((uint32_t)reg_dbr) << 16) & 0x00ffffff;
            src_value += reg_y.reg_y;
            operands[0] = memory_pointer(src_value);
            // dereference_src = 1;
...
//            addr_mode_str = "unknown";
    ...
    // reg_temp0 = dereference_src ? *(uint32_t *)memory_pointer(src_value) : src_value;
        ...
        case OP_CODE_CATEGORY_ARITHMETIC:
            ...
            switch(opcode.opcode)
            {
                ...
                case OP_CODE_EOR:
                    ...
                    /* if the accumlator is in 16 bit mode, the signal bit will
                    be bit 15, otherwise will be bit 7 */
                    uint32_t signal_mask = byte_write ? 0x00000080 : 0x00008000;
                    /* if the accumulator is in 16 bit mode, carry will happen if
                    bit 16 gets set, otherwise will be bit 8 */
                    uint32_t carry_mask = byte_write ? 0x00001000 : 0x0010000;
                    uint32_t zero_mask = byte_write ? 0x000000ff : 0x0000ffff;

                    uint32_t overflow = (reg_p & CPU_STATUS_FLAG_OVERFLOW) && 1;
                    uint32_t carry = (reg_p & CPU_STATUS_FLAG_CARRY);

                    reg_temp0 = *(uint16_t *)operands[0];

                    switch(opcode.opcode)
                    {
                        case OP_CODE_SBC:
                            reg_temp0 = -((int32_t)reg_temp0);
                            carry = -((int32_t)!carry);

                        case OP_CODE_ADC:
                            reg_temp1 = (int32_t)reg_accum.reg_accumC + (int32_t)reg_temp0 + carry;
                            /* SBC does not affect overflow flag */
                            /* (((!(a ^ b)) & (b ^ r)) & signal_mask) */
                            overflow = opcode.opcode == OP_CODE_ADC ? (((!((uint32_t)reg_accum.reg_accumC ^ reg_temp0)) & (reg_temp0 ^ reg_temp1)) & signal_mask) : overflow;
                        break;
                        ...
                        case OP_CODE_ASL:
                        case OP_CODE_LSR:
                            reg_temp1 = opcode.opcode == OP_CODE_ASL ? reg_temp0 << 1 : reg_temp0 >> 1;
                            ...
                            // if(opcode.address_mode == ADDRESS_MODE_ACCUMULATOR)
                            // {
                            //     reg_temp = opcode.opcode == OP_CODE_ASL ? reg_accum.reg_accumC << 1 : reg_accum.reg_accumC >> 1;
                            ...
                            //     reg_temp = (*(unsigned short *)(opcode_address));
                        ...
                        case OP_CODE_AND:
                            reg_temp1 = (uint32_t)reg_accum.reg_accumC & reg_temp0;
                        break;
                    ...
                    reg_p = overflow ? (reg_p | CPU_STATUS_FLAG_OVERFLOW) : (reg_p & (~CPU_STATUS_FLAG_OVERFLOW));
                    reg_p = (reg_temp1 & carry_mask) ? (reg_p | CPU_STATUS_FLAG_CARRY) : (reg_p & (~CPU_STATUS_FLAG_CARRY));
                    reg_p = (reg_temp1 & zero_mask) ? (reg_p & (~CPU_STATUS_FLAG_ZERO)) : (reg_p | CPU_STATUS_FLAG_ZERO); 
                    reg_p = (reg_temp1 & signal_mask) ? (reg_p | CPU_STATUS_FLAG_NEGATIVE) : (reg_p & (~CPU_STATUS_FLAG_NEGATIVE));

                    if(byte_write)
                    {
                        reg_accum.reg_accumA = reg_temp1;
                    }
                    else
                    {
                        reg_accum.reg_accumC = reg_temp1;
                    }
                break;
    ...
    //     if(byte_copy)
    //     {
    //         *dst_adress = reg_temp1;
    //     }
    //     else
    //     {
    //         *(uint16_t *)dst_address = reg_temp1;
    //     }
    //     // memcpy(dst_address, &reg_temp1, byte_copy ? 1 : 2);
    // }
}
